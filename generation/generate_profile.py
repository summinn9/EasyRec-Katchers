...
REGENERATE_ITEM = False   # 기존 item LLM 재사용
REGENERATE_USER = True    # user LLM 새로 생성
...
item_system_prompt = load_instruction(os.path.join(INSTRUCTION_DIR, "item_system_prompt.txt"))
user_system_prompt = load_instruction(os.path.join(INSTRUCTION_DIR, "user_system_prompt.txt"))
...
print(f"item 최종 결과 수: {len(item_profiles_llm)}")
...
print(f"user fallback 처리 수: {user_skipped_fallback_count}")
print(f"user 빈 입력 처리 수: {user_skipped_empty_count}")
...
print(f"user 최종 결과 수: {len(user_profiles_llm)}")
